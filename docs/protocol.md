...
```
k          the person's key: 64 lowercase hex chars (32 random bytes).
           Generated by `make invite`. Lives in the ESP32 roster and in that
           person's clients. Not derived from anything.
...
Note the HMAC **key is the 64 ASCII bytes of the hex string `k`**, not 32 raw
bytes. This is deliberate — every client platform can use a hex string as an
HMAC key; not all can pass raw bytes.

If you would rather a person have a memorable passphrase, set their `k` to
`sha256hex(passphrase)` (`printf %s 'passphrase' | sha256sum`) instead of a
random value. The firmware and protocol are identical either way — they only
ever see `k`.

## Setup link

`make invite` also prints a personal link the person taps once:

```
<WEB_BASE_URL>/#n=<url-encoded name>&k=<k>
```

The web page reads it, stores `name` + `k` in `localStorage`, and scrubs the
fragment from the URL. A query string (`?n=…&k=…`) is also accepted.
...
(`k` here happens to be `sha256("hunter2")`.) Test any implementation with:

```
node firmware/tools/send-open.mjs --name Tomek \
  --key f52fbd32b2b3b86ff88ef6c490628285f482af15ddcb29541f94bcf526a3f6c7 --ts 1700000000
```
