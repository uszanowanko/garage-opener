// Root CAs for ntfy.sh (Let's Encrypt). Both the RSA root (ISRG Root X1, valid
// to 2035) and the ECDSA root (ISRG Root X2, valid to 2040) are included so a
// chain switch on the server side does not brick the device. mbedTLS parses a
// concatenated PEM bundle fine.
//
// If Let's Encrypt ever rotates to a new root, or you self-host ntfy with a
// different CA: update this file and OTA the firmware. See docs/self-host-ntfy.md.
#pragma once

static const char NTFY_ROOT_CA_BUNDLE[] PROGMEM = R"EOF(
-----BEGIN CERTIFICATE-----
...
oyi3B43njTOQ5yOf+1CceWxG1bQVs5ZufpsMljq4Ui0/1lvh+wjChP4kkqOJ2qxq
